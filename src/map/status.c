...
#define SCDEF_LVL_DIFF(bl, src, maxlv, factor) ( ( SCDEF_LVL_CAP((bl), (maxlv)) - SCDEF_LVL_CAP((src), (maxlv)) ) * (factor) )
	...
	case SC_FREEZE:
		...
		//tentativa 2 95%
		/*sc_def = st->mdef*100 + st->luk*5;
        sc_def2 = st->luk*10 + SCDEF_LVL_DIFF(bl, src, 99, 10);
        tick_def = st->luk*3 + st->mdef*90;
		tick_def2 = st->luk*3 + st->mdef*90;*/
		//Tentativa 3 90%
		////sc_def = st->mdef*100;
		//sc_def2 = st->luk*10 + SCDEF_LVL_DIFF(bl, src, 99, 10);
...
int status_readdb_refine_libconfig_sub(config_setting_t *r, const char *name, const char *source) {
	...
	if ((rate = libconfig->setting_get_member(r, "Rates")) && config_setting_is_group(rate)) {
		...
		for (i = 0; i < MAX_REFINE; i++) {
			status->refine_info[type].chance[i] = chance_[i] ? chance_[i] : 100;
			status->refine_info[type].randombonus_max[i] = rnd_bonus[i];
			bonus_[i] += i ? bonus_[i - 1] : 0;
			status->refine_info[type].bonus[i] = bonus_[i] ? bonus_[i] : 0;
		}
	}
	else {
